...
While other tools exist with a similar purpose, this one puts emphasis on easy/fast development and real-time performance (e.g. game engines).
...
1. Download the latest binary from the "releases" tab, or build the tool from source (see build instructions below).
2. Include the header file from the test program ([rabbitcall.h](test/cpp_prj/src/rabbitcall/rabbitcall.h)) in your C++ project's headers.
3. Copy the configuration file from the test program ([rabbitcall.xml](test/rabbitcall.xml)) to your project folder. See the instructions in the file and update configuration settings if necessary (set paths, remove unnecessary type mappings).
...
See [implementation notes](rabbitcall/implementation_notes.md).
